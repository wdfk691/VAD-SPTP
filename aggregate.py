...
def video_label_length(dataset='shanghaitech'):
    if dataset == 'shanghaitech':
        label_path = "E:/WL/Jigsaw-VAD-main/irip/wangguodong_2020/projects/datasets/vad/shanghaitech/frame_masks/"
        video_length = {}
        ...
        test_frame_path = 'E:/WL/Jigsaw-VAD-main/irip/wangguodong_2020/projects/datasets/vad/' + dataset + '/testing/'
...
def load_objects(dataset, frame_num=7):
    root = 'E:/WL/Jigsaw-VAD-main/irip/wangguodong_2020/projects/datasets/vad'
...
def remake_video_output(video_output, dataset='ped2'):
    ...
    for i in range(len(video_l)):
        ...
        #video2_[non_ones] = (video2_[non_ones] - min(video2_)) / (local_max2_ - min(video2_))#原
        ...
        if non_ones.size > 0:
             local_max_ = max(video_[non_ones])
             min_video_ = min(video_)
             if local_max_ != min_video_:
                video_[non_ones] = (video_[non_ones] - min_video_) / (local_max_ - min_video_)
             else:
                  video_[non_ones] = 0  # or another value to handle the case where local_max_ == min_video_

         # temporal
        non_ones = (video2_ != 1).nonzero()[0]
        if non_ones.size > 0:
             local_max2_ = max(video2_[non_ones])
             min_video2_ = min(video2_)
             if local_max2_ != min_video2_:
                  video2_[non_ones] = (video2_[non_ones] - min_video2_) / (local_max2_ - min_video2_)
             else:
                  video2_[non_ones] = 0  # or another value to handle the case where local_max2_ == min_video2_
